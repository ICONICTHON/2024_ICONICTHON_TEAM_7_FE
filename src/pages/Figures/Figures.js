          ...
          <div className={styles.top}>
            <div className={styles.currentRoom}>
              ...
              <div className={styles.title}>
                ...
                />
                <div style={{ fontSize: "34px" }}>{buildingName}</div>
              </div>
              <NEBDropdown
                onSelect={handleNEBSelect}
                defaultValue={selectedOption}
              />
            </div>
            <TopBox
              image="tempIcon.png"
              value={loading ? "--" : sensorData.Temperature?.value}
              unit="℃"
              name="temperature"
            />
            <TopBox
              image="humidIcon.png"
              value={loading ? "--" : sensorData.Humidity?.value}
              unit="%"
              name="humidity"
            />
            <TopBox
              image="TVOCIcon.png"
              value={loading ? "--" : sensorData.TVOC?.value}
              unit="㎍/m³"
              name="TVOC"
            />
            <TopBox
              image="PM25Icon.png"
              value={loading ? "--" : sensorData.PM2_5MassConcentration?.value}
              unit="㎍/m³"
              name="PM2.5"
            />
            <TopBox
              image="noise.png"
              value={loading ? "--" : sensorData.AmbientNoise?.value}
              unit="db"
              name="noise"
            />
          </div>
            ...
            <div className={styles.rightContainer}>
              ...
              <div className={styles.toggleContainer}>
                <div className={styles.toggleTitles}>
                  ...
                  <span
                    className={`${styles.toggleTitle} ${
                      activeComponent === "controlLog" ? styles.activeTitle : ""
                    }`}
